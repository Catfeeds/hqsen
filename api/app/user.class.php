...
//                      order by hhr.hotel_weight asc  , hh.id asc");
    ...
    public function syncOrder(){
        $start_time = time() - 3600 * 24 * 5; //同步三天前的订单
        // 获取三天前 未同步的指定酒店的订单
    ...
    public function syncHotelOrder($order, $using_hotel_id)
    {
        ...
        foreach ($user_data as $one_user_data) {
            ...
            $old_order = $this->db->getRow('select * from hqsen_user_kezi_order where kezi_order_id =' . $order['id'] . " and watch_user_hotel_name='" . $one_user_data['hotel_name'] . "'");
            if($old_order){
                continue;
            }
            if ($one_user_data) {
